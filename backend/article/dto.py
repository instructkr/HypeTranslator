...
async def from_model(model: ArticleModel) -> ArticleDTO:
    ...
    '''
    related_to_organizer: OrganizerModel = await model.awaitable_attrs.related_to_organizer
    return ArticleDTO(
        article_id=model.article_id,
        url=model.url,
        author=model.author,
        published_at=model.published_at,
        related_to_organizer=
            organizer_from_model(related_to_organizer)
            if related_to_organizer else None
    )
